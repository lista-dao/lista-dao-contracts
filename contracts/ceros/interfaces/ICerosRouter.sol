...
interface ICerosRouter {
  ...
  event Deposit(address indexed account, address indexed token, uint256 amount, uint256 profit);

  event Claim(address indexed recipient, address indexed token, uint256 amount);
  ...
  // in aBNBc
  function depositABNBcFrom(address owner, uint256 amount) external returns (uint256);
  ...
  // BNB
  function withdrawFor(address recipient, uint256 amount) external returns (uint256);
  ...
  // aBNBc
  function withdrawABNBc(address recipient, uint256 amount) external returns (uint256);
}
