...
import { ICerosRouter } from "./interfaces/ICerosRouter.sol";
import { IHelioProvider } from "./interfaces/IHelioProvider.sol";
  ...
  ReentrancyGuardUpgradeable
{
  ...
  modifier onlyProxy() {
    require(msg.sender == owner() || msg.sender == _proxy, "AuctionProxy: not allowed");
    _;
  }
   ...
   * DEPOSIT
   */
  function provide() external payable override whenNotPaused nonReentrant returns (uint256 value) {
    ...
    returns (uint256 realAmount)
  {
    ...
    require(amount >= minumumUnstake, "value must be greater than min unstake amount");
   ...
   * DAO FUNCTIONALITY
   */
  function liquidation(address recipient, uint256 amount) external override onlyProxy nonReentrant {
    _ceRouter.withdrawABNBc(recipient, amount);
  }

  function daoBurn(address account, uint256 value) external override onlyProxy nonReentrant {
    _collateralToken.burn(account, value);
  }

  function daoMint(address account, uint256 value) external override onlyProxy nonReentrant {
