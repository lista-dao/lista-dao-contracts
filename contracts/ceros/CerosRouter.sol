  ...
  ReentrancyGuardUpgradeable
{
  ...
  modifier onlyProvider() {
    require(msg.sender == owner() || msg.sender == _provider, "Provider: not allowed");
    _;
  }
   ...
   * DEPOSIT
   */
  function deposit() external payable override nonReentrant returns (uint256 value) {
    ...
    if (poolABNBcAmount >= dexABNBcAmount) {
      ...
      uint256[] memory amounts = _dex.swapExactETHForTokens{ value: amount }(
        dexABNBcAmount,
        path,
        address(this),
        block.timestamp + 300
  ...
  function depositABNBc(uint256 amount) external override nonReentrant returns (uint256 value) {
  ...
  function claim(address recipient) external override nonReentrant returns (uint256 yields) {
  ...
  function claimProfit(address recipient) external nonReentrant {
    ...
    // let's check balance of CeRouter in aBNBc
    require(_certToken.balanceOf(address(this)) >= profit, "insufficient amount");
  ...
  {
    require(amount >= _pool.getMinimumStake(), "value must be greater than min unstake amount");
