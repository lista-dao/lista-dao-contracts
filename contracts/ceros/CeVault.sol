...
contract CeVault is IVault, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  /**
   * Variables
   */
  ...
  // deposit
  function _deposit(address account, uint256 amount) private returns (uint256) {
  ...
  // claimYields
  function claimYields(address recipient) external override nonReentrant returns (uint256) {
  ...
  function _claimYields(address owner, address recipient) private returns (uint256) {
    uint256 availableYields = getYieldFor(owner);
  ...
  // 100 abnbc ratio - 1 === 100 BNB
  // after one year ratio was changed to 0.9
  // we can get 90 abnbc === 100 BNB
  // and the remaining 10 abnbc will be gone to the helio
  function getYieldFor(address account) public view override returns (uint256) {
    uint256 principal = getPrincipalOf(account);
  ...
  function getPrincipalOf(address account) public view override returns (uint256) {
    uint256 ratio = _aBNBc.ratio();
  ...
  function _withdraw(
    ...
    require(_aBNBc.balanceOf(address(this)) >= realAmount, "not such amount in the vault");
