...
contract ResilientOracleMock {
  ...
  BoundValidatorInterfaceMock public immutable boundValidator;
  ...
  function _getPrice(address asset) internal view returns (uint256) {
    ...
    if (pivotOracleEnabled && pivotOracle != address(0)) {
      try OracleInterfaceMock(pivotOracle).getPrice(asset) returns (uint256 pricePivot) {
  ...
  function _getMainOraclePrice(
    ...
    if (mainOracleEnabled && mainOracle != address(0)) {
      try OracleInterfaceMock(mainOracle).getPrice(asset) returns (uint256 mainOraclePrice) {
        if (!pivotEnabled) {
  ...
  function _getFallbackOraclePrice(address asset, uint256 pivotPrice) private view returns (uint256, bool) {
    ...
    if (fallbackEnabled && fallbackOracle != address(0)) {
      try OracleInterfaceMock(fallbackOracle).getPrice(asset) returns (uint256 fallbackOraclePrice) {
        if (pivotPrice == INVALID_PRICE) {
